...
try {
  ...
  $results = $youtube->liveStreams->listLiveStreams('id,snippet,cdn,status', array(
      'mine' => 'true',
  ));
...
if(isset($results)){
  ...
  foreach ($results['items'] as $streamItem) {
    echo("<li>" . $streamItem['snippet']['title'] . " (" . $streamItem['id'] . ") broadcast id (" . $streamItem['cdn']['ingestionInfo']['streamName'] . ") <a href='delete-stream.php?id=" . $streamItem['id'] . "'>x</a></li>");
  }
  echo('</ul>');
}
